...
class WaitingPostMembers_fetch_belowList_widget extends StatelessWidget {
  final scrollController;
  final scrollListener;
  final post_fk;
  const WaitingPostMembers_fetch_belowList_widget({
...
/* -------------------------------------------------------------------------- */
  ...
  belowLastTile_STATES() {
    return BlocConsumer<WaitingPostMembersFetchBloc,
        WaitingPostMembersFetchState>(
      listener: (context, state) {
        if (state is WaitingPostMembers_Fetch_More_Loaded_ButEmpty_State) {
      ...
      builder: (context, state) {
        if (state is WaitingPostMembers_Fetch_Loading_State) {
          return customLoadingWidget();
        } else if (state is WaitingPostMembers_Fetch_Error_State) {
          return connectionErrorText(context, state.error);
        } else if (state
            is WaitingPostMembers_Fetch_More_Loaded_ButEmpty_State) {
    ...
    return TextButton(
      ...
      onPressed: () {
        BlocProvider.of<WaitingPostMembersFetchBloc>(context)
            .add(WaitingPostMembers_Fetch_onInit_Event(post_fk));
      },
    );
  }
